(()=>{var r=(m,o,n)=>new Promise((e,a)=>{var s=c=>{try{l(n.next(c))}catch(t){a(t)}},v=c=>{try{l(n.throw(c))}catch(t){a(t)}},l=c=>c.done?e(c.value):Promise.resolve(c.value).then(s,v);l((n=n.apply(m,o)).next())});var u=class{constructor(o){this.apiBaseUrl=o||"/api/database",this.cache={events:null,communications:null,staff:null}}fetchCollection(o){return r(this,null,function*(){if(this.cache[o])return this.cache[o];try{let e=yield(yield fetch(`${this.apiBaseUrl}/index.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({functionname:"get",collection:o,arguments:{}})})).json();if(e.success)return this.cache[o]=e.result,e.result;throw new Error(e.error||"Failed to fetch collection")}catch(n){throw console.error(`Error fetching ${o}:`,n),n}})}getEvents(){return r(this,null,function*(){return this.fetchCollection("events")})}getCommunications(){return r(this,null,function*(){return this.fetchCollection("communications")})}getStaff(){return r(this,null,function*(){return this.fetchCollection("staff")})}formatDate(o){return new Date(o).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"})}formatDateTime(o){return new Date(o).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric",hour:"numeric",minute:"2-digit",hour12:!0})}parseMarkdown(o){return o?o.replace(/\*\*(.+?)\*\*/g,"<strong>$1</strong>").replace(/\*(.+?)\*/g,"<em>$1</em>").replace(/\n/g,"<br>"):""}renderEvents(o=".collections-section.events"){return r(this,null,function*(){let n=document.querySelector(o);if(!n)return;let e=n.querySelector(".collections-layout.events");e&&(e.innerHTML='<div class="loading">Loading events...</div>');try{let a=yield this.getEvents();if(!a||a.length===0){e.innerHTML='<div class="no-data">No events available</div>';return}a.sort((i,d)=>new Date(i.date)-new Date(d.date));let s=a[0],v=`
            ...
            `,c=`
                <div class="event-list">
                    ${a.map((i,d)=>`
                <div class="event-item ${d===0?"active":""}" 
                     data-event-index="${d}" 
            ...
            `).join(""),s=n.querySelector(".events-data");s&&(s.innerHTML=a);let v=e.map((t,i)=>{let d=new Date(t.date),h=d.getDate(),p=d.toLocaleDateString("en-US",{month:"short"}),y=d.toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0}),w=d.getFullYear();return`
            ...
            `:"",c=n.querySelector(".event-cards-container");c&&(c.innerHTML=v+l),window.dispatchEvent(new CustomEvent("calendarDataLoaded",{detail:{events:e}}))}catch(e){console.error("Error loading calendar events:",e);let a=n.querySelector(".events-data");a&&(a.innerHTML=`<div class="error">Error loading events: ${e.message}</div>`);let s=n.querySelector(".event-cards-container");s&&(s.innerHTML=`<div class="error">Error loading events: ${e.message}</div>`)}})}escapeHtml(o){let n=document.createElement("div");return n.textContent=o,n.innerHTML}renderCommunications(o=".collections-section.communications"){return r(this,null,function*(){let n=document.querySelector(o);if(!n)return;let e=n.querySelector(".communication-list");e&&(e.innerHTML='<div class="loading">Loading communications...</div>');try{let a=yield this.getCommunications();if(!a||a.length===0){e.innerHTML='<div class="no-data">No communications available</div>';return}a.sort((t,i)=>new Date(i.created_at)-new Date(t.created_at));let s=a.map((t,i)=>`
